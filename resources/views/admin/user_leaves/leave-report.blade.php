...
@section('content')
    @if(empty($url))
        <input type="hidden" id="page_url" value="{{ route('user_leaves.report') }}">
    @else
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center row">
                            ...
                            <a href="{{ route('employees.show', $user->slug) }}" class="text-body text-truncate">
                                <div class="d-flex align-items-center">
                                    @if(isset($user->profile) && !empty($user->profile->profile))
                                        <img src="{{ resize(asset('public/admin/assets/img/avatars').'/'.$user->profile->profile) }}" style="width:40px !important; height:40px !important" alt class="h-auto rounded-circle" />
                                    @else
                    ...
                    <div class="col-md-4">
                            ...
                            <div class="row">
                                <div class="col-lg-12 col-md-12">
                                    <select class="select2 form-select" id="redirectDropdown" onchange="redirectPage(this)" required>
                                        <option value="{{ route('user_leaves.report') }}" selected>Select All</option>
                                        @foreach ($employees as $employee)
                                            <option data-user-slug="{{ $employee->slug }}" value="{{ URL::to('user_leaves/report/'.$employee->slug) }}" {{ $user->slug==$employee->slug?"selected":"" }}>
                        ...
                        @php $url = URL::to('user_leaves/report') @endphp
                        @include('admin.layouts.employee-dropdown', ['employees' => $employees, 'user' => $user, 'url' => $url, 'type' => 'redirect'])
                    </div>
                </div>
