            ...
            <div class="card">
                ...
                <div class="card-header d-flex justify-content-between align-items-center row">
                    ...
                    <div class="col-md-4 mt-md-0 mt-3">
                        @php $url = URL::to('user/attendance/summary') @endphp
                        @include('admin.layouts.employee-dropdown', ['employees' => $employees, 'user' => $user, 'url' => $url, 'month' => $month, 'year' => $year, 'type' => 'redirect'])
                    </div>
                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-6 col-lg-2 col-md-4">
                                        <span
                                            ...
                                            @if(count($statistics['lateInDates']) > 0 && ($isCurrentTimeInRange || Auth::user()->hasRole('Admin'))) data-bs-toggle="modal" data-bs-target="#teamlateinModal" class="late-in-box" @endif
                                    ...
                                    <div class="col-sm-6 col-lg-2 col-md-4">
                                        <span
                                            data-user-id="{{ $user->id }}"
                                            @if(count($statistics['earlyOutDates']) > 0 && ($isCurrentTimeInRange || Auth::user()->hasRole('Admin'))) data-bs-toggle="modal" data-bs-target="#teamEarlyOutModal" class="early-out-box" @endif
                                            @if(Auth::user()->slug != $user->slug) data-user="false" @else data-user="true" @endif
                                    ...
                                    <div class="col-sm-6 col-lg-2 col-md-4">
                                        <span
                                            data-user-id="{{ $user->id }}"
                                            @if(count($statistics['halfDayDates']) > 0 && ($isCurrentTimeInRange || Auth::user()->hasRole('Admin'))) data-bs-toggle="modal" data-bs-target="#teamhalfdayModal" class="half-day-box" @endif
                                            @if(Auth::user()->slug != $user->slug) data-user="false" @else data-user="true" @endif
                                    ...
                                    <div class="col-sm-6 col-lg-2 col-md-4">
                                        <span
                                            data-user-id="{{ $user->id }}"
                                            @if(count($statistics['absent_dates']) > 0 && ($isCurrentTimeInRange || Auth::user()->hasRole('Admin'))) data-bs-toggle="modal" data-bs-target="#teamabsentModal" class="absent-dates-box" @endif
                                            @if(Auth::user()->slug != $user->slug) data-user="false" @else data-user="true" @endif
                            ...
                            <tbody>
                                ...
                                @for($i = $begin; $i <= $end; $i->modify('+1 day'))
                                    ...
                                    @if($reponse!=null)
                                        ...
                                        @endphp
                                        @php
                                            $attendance_adjustment = attendanceAdjustment($user->id, $reponse['attendance_id'], $i->format("Y-m-d"));
                                            ...
                                            <tr class="{{$day}}">
                                                ...
                                                <td>
                                                    ...
                                                    @if($day!='Sat' && $day!='Sun')
                                                        ...
                                                        @if((Auth::user()->hasRole('Admin')) && $start_date <= $begin)
                                                                    ...
                                                                    @if($attendance_adjustment->mark_type=='lateIn' || $attendance_adjustment->mark_type=='firsthalf' || $attendance_adjustment->mark_type=='absent')
                                                                        ...
                                                                        @if($attendance_adjustment=='firsthalf' || $attendance_adjustment=='absent')
                                                                            @php
                                                                                ...
                                                                                $apply_date = date('d-m-Y', strtotime($i->format("Y-m-d")));
                                                                            @endphp
                                                                        @else
                                                                            @php
                                                        ...
                                                        @elseif($remaining_filable_leaves > 0)
                                                            @if($i->format("Y-m-d") <= date('Y-m-d'))
                                                                ...
                                                                @else
                                                                    ...
                                                                    @endphp
                                                                @endif
                                                                @if((checkAdjustedAttendance($user->id, date('Y-m-d', strtotime($nextDate))) && checkAdjustedAttendance($user->id, date('Y-m-d', strtotime($previousDate)))) && checkAttendance($user->id, date('Y-m-d', strtotime($nextDate)), date('Y-m-d', strtotime($secondNextDate)), $shift) && checkAttendance($user->id, date('Y-m-d', strtotime($previousDate)), $i->format("Y-m-d"), $shift))
                                                                    <a href="#"
                                                                        class="badge bg-label-danger"
                                                                        id="custom-add-btn"
                                                                        tabindex="0" aria-controls="DataTables_Table_0"
                                                ...
                                                @if(Auth::user()->hasRole('Admin') || Auth::user()->hasPermissionTo('mark_attendance-create'))
                                                    <td>
                                                    @if($day!='Sat' && $day!='Sun')
