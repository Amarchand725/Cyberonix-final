...
class TicketController extends Controller
{
    ...
    public function index(Request $request , $user_slug = null)
    {
        ...
        $title = 'My Tickets';
        $data['ticket_categories'] = getTicketCategoriesAndReasons()['ticket_categories'];
        $data['reasons'] = getTicketCategoriesAndReasons()['reasons'];
    ...
    public function teamTickets(Request $request, $user_slug = null){
        ...
        // $departments = Department::where('manager_id', $logined_user->id)->get();
        ...
        $data['employees'] = getTeamMembers($user);
        if(!empty($user_slug) && $user_slug != 'All'){
            ...
            $model = Ticket::orderby('id', 'desc')->whereIn('user_id', $employees_ids)->get(); //4 = Fleet no need for manager just only for admin.
        }
    ...
    public function adminTeamTickets(Request $request, $user_slug = null){
        ...
        if(!empty($user_slug) && $user_slug != 'All'){
            ...
            $tickets = Ticket::orderby('id', 'desc')->where('user_id', $user->id)->get();
            $all_tickets = [];
            foreach($tickets as $ticket){
                if($ticket->ticket_category_id==4 || $ticket->ticket_category_id==2){ //Fleet and it rapid.
                    $all_tickets[] = $ticket;
            ...
            $employees_ids = getTeamMemberIds($user);
            $tickets = Ticket::orderby('id', 'desc')->whereIn('user_id', $employees_ids)->get();
            $all_tickets = [];
            foreach($tickets as $ticket){
                if($ticket->ticket_category_id==4 || $ticket->ticket_category_id==2){ //Fleet and it rapid.
                    $all_tickets[] = $ticket;
    ...
    public function allTickets(Request $request){
        ...
        if($department->name=="IT Department"){
            ...
            foreach($all_models as $al_model){
                if($al_model->ticket_category_id==1 && !empty($al_model->is_concerned_approved) && !empty($al_model->is_manager_approved)){ //get approved by admin and manager// IT Equipment
                    $it_tickets[] = $al_model;
                }elseif($al_model->ticket_category_id==2 && !empty($al_model->is_manager_approved)){ //get approved by manager //IT Rapid Support
                    $it_tickets[] = $al_model;
                }
            }
            ...
            $all_models = Ticket::orderby('id', 'desc')->where('ticket_category_id', 3)->get(); // Finance
            $it_tickets = [];
            foreach($all_models as $al_model){
    ...
    public function adminAllTickets(Request $request){
        ...
        // foreach($department_users as $department_user){
        //     $emp_data = User::where('id', $department_user->user_id)->where('id', '!=', $user->id)->first(['id','first_name', 'last_name', 'slug']);
        ...
        // $all_tickets = [];
        // foreach($tickets as $ticket){
        //     if($ticket->ticket_category_id==4){ //Fleet and it rapid.
        //         $all_tickets[] = $ticket;
        ...
        $model = Ticket::orderby('id', 'desc')->get();
        if($request->ajax() && $request->loaddata == "yes") {
            return DataTables::of($model)
                ->addIndexColumn()
                ->editColumn('status', function ($model) {
        ...
        return view('admin.tickets.admin_all_tickets', compact('title'));
    }
    ...
    public function waitingApprovalTickets(Request $request){
        ...
        $employees_ids = getTeamMemberIds($user);
        $all_it_tickets = Ticket::orderby('id', 'desc')->whereHas('hasEmployee')->whereIn('ticket_category_id', [1, 4])->get(); //IT Equipment
        $model = [];
